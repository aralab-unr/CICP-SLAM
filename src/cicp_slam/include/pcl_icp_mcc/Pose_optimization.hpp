...
//#include <pcl_icp_mcc/graph_slam.hpp>
//#include <pcl_icp_mcc/keyframe.hpp>
//#include <pcl_icp_mcc/keyframe_updater.hpp>
//#include <pcl_icp_mcc/loop_detector.hpp>
//#include <pcl_icp_mcc/information_matrix_calculator.hpp>
//#include <pcl_icp_mcc/map_cloud_generator.hpp>
 ...
 ~PoseOptimizationNode() {}
  ...
  void onInit() 
  {
   ...
   // private_nh = getPrivateNodeHandle();
    ...
    max_keyframes_per_update = private_nh.param<int>("max_keyframes_per_update",100);

    //
    anchor_node = nullptr;
    anchor_edge = nullptr;
    floor_plane_node = nullptr;
    graph_slam.reset(new GraphSLAM(private_nh.param<std::string>("g2o_solver_type", "lm_var_cholmod")));
    ...
    enable_imu_orientation = private_nh.param<bool>("enable_imu_orientation", true);
    enable_imu_acceleration = private_nh.param<bool>("enable_imu_acceleration", true);
    imu_orientation_edge_stddev = private_nh.param<double>("imu_orientation_edge_stddev", 0.0);
    imu_acceleration_edge_stddev = private_nh.param<double>("imu_acceleration_edge_stddev", 0.0 );

    points_topic = private_nh.param<std::string>("points_topic", "/kitti/velo/pointcloud");

    // subscribers
    odom_sub.reset(new message_filters::Subscriber<nav_msgs::Odometry>(mt_nh, "/odom", 10000));
    cloud_sub.reset(new message_filters::Subscriber<sensor_msgs::PointCloud2>(mt_nh, "/filtered_points", 10000));
    sync.reset(new message_filters::TimeSynchronizer<nav_msgs::Odometry, sensor_msgs::PointCloud2>(*odom_sub, *cloud_sub, 10000));
    sync->registerCallback(boost::bind(&PoseOptimizationNode::cloud_callback, this, _1, _2));
    imu_sub = nh.subscribe("/kitti/oxts/imu", 1024, &PoseOptimizationNode::imu_callback, this);
   // floor_sub = nh.subscribe("/floor_detection/floor_coeffs", 1024, &HdlGraphSlamNodelet::floor_coeffs_callback, this);
    ...
    // publishers
    markers_pub = mt_nh.advertise<visualization_msgs::MarkerArray>("/pcl_icp_mcc/markers", 1000);
    odom2map_pub = mt_nh.advertise<geometry_msgs::TransformStamped>("/pcl_icp_mcc/odom2pub", 1000);
    map_points_pub = mt_nh.advertise<sensor_msgs::PointCloud2>("/pcl_icp_mcc/map_points", 1000);
    read_until_pub = mt_nh.advertise<std_msgs::Header>("/pcl_icp_mcc/read_until", 1000);

    dump_service_server = mt_nh.advertiseService("/pcl_icp_mcc/dump", &PoseOptimizationNode::dump_service, this);
    save_map_service_server = mt_nh.advertiseService("/pcl_icp_mcc/save_map", &PoseOptimizationNode::save_map_service, this);

    double graph_update_interval = private_nh.param<double>("graph_update_interval", 15.0);
    double map_cloud_update_interval = private_nh.param<double>("map_cloud_update_interval", 20.0);
    optimization_timer = mt_nh.createWallTimer(ros::WallDuration(graph_update_interval), &PoseOptimizationNode::optimization_timer_callback, this);
    map_publish_timer = mt_nh.createWallTimer(ros::WallDuration(map_cloud_update_interval), &PoseOptimizationNode::map_points_publish_timer_callback, this);
  }
...
private:
  ...
  void cloud_callback(const nav_msgs::OdometryConstPtr& odom_msg, const sensor_msgs::PointCloud2::ConstPtr& cloud_msg) 
  {
    ...
    //std::cout<<"\nkeyfr_update="<<keyframe_updater->update(odom);
    std::cout<<"\n keyframe_size="<<keyframe_queue.size();
    if(!keyframe_updater->update(odom)) 
    {
      ...
      if(keyframe_queue.empty()) {
        ...
        read_until.stamp = stamp + ros::Duration(0.1, 0);
  ...
  bool flush_keyframe_queue() 
  {
    ...
    int num_processed = 0;
     //std::cout<<"A_keyframeSize="<<keyframes.size();
    for(int i=0; i<std::min<int>(keyframe_queue.size(), max_keyframes_per_update); i++) 
    {
      ...
      if(keyframes.empty() && new_keyframes.size() == 1) 
      {
       ...
       std::cout<<"\ni="<<i<<"kf="<<keyframes.empty();
      if(i==0 && keyframes.empty()) {
        continue;
      }

      // add edge between consecutive keyfrfalse
      ...
      graph_slam->add_robust_kernel(edge, private_nh.param<std::string>("odometry_edge_robust_kernel", "Cauchy"), private_nh.param<double>("odometry_edge_robust_kernel_size", 2.0));
    }

    std_msgs::Header read_until;
    read_until.stamp = keyframe_queue[num_processed]->stamp + ros::Duration(0.1, 0);
    read_until.frame_id = points_topic;
    read_until_pub.publish(read_until);
    read_until.frame_id = "/filtered_points";
    read_until_pub.publish(read_until);
  ...
  void optimization_timer_callback(const ros::WallTimerEvent& event) 
  {
    ...
    if(!keyframe_updated) {
      std_msgs::Header read_until;
      read_until.stamp = ros::Time::now() + ros::Duration(20, 0);
      read_until.frame_id = points_topic;
      read_until_pub.publish(read_until);
      read_until.frame_id = "/filtered_points";
      read_until_pub.publish(read_until);
    ...
    for(const auto& loop : loops) 
    {
      ...
      graph_slam->add_robust_kernel(edge, private_nh.param<std::string>("loop_closure_edge_robust_kernel", "Huber"), private_nh.param<double>("loop_closure_edge_robust_kernel_size", 1.0));
    }
    ...
    // publish tf
    //std::cout<<"keyframes_size="<<keyframes.size();

    const auto& keyframe = keyframes.back();
    Eigen::Isometry3d trans = keyframe->node->estimate() * keyframe->odom.inverse();
    std::cout<<"\nkf_size="<<keyframes.size();
    std::cout<<"\ntrans= "<<keyframe->node->estimate().translation();
    trans_odom2map_mutex.lock();
  ...
  visualization_msgs::MarkerArray create_marker_array(const ros::Time& stamp) const {
    ...
    traj_marker.header.frame_id = "velo_link";
    ...
    edge_marker.header.frame_id = "velo_link";
    ...
    sphere_marker.header.frame_id = "velo_link";
